...
class UserDB {
    ...
    async addOrUpdateCartItemByEmail(email, item) {
        ...
        try {
            ...
            const result = await client.db(this.db_name).collection(this.collection).updateOne({ email, "cart.productId": item.productId }, { $set: { "cart.$": item } }, // Update existing item in cart array
